...
1. Agentic Readiness Diagnostic - $2,500 to $5,000, two weeks. The exact figure sits inside the range by the scope of the workflow, is fixed in writing before the engagement starts, and never moves mid-engagement. One workflow analyzed; written findings report with severity ratings and evidence: where agents fit, where they do not, proposed architecture, build cost estimate. If the answer is "do not build this," the report says so. This is the only public price; all other work is scoped after the diagnostic.
...
flowchart TD
    ...
    subgraph L2["LAYER 2 — Production Line (Claude Code)"]
    ...
    subgraph L3["LAYER 3 — Audience Critique Council (Antigravity)"]
